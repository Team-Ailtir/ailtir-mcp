...
async def test_list_returns_formatted_kbs(mock_ctx: MagicMock) -> None:
    respx.get("http://test-mcp-api/kb").mock(
        return_value=Response(
            200,
            json=[
                {"kb_id": "abc-1", "name": "Tender Q1", "status": "ready"},
                {"kb_id": "abc-2", "name": "Tender Q2", "status": "analysing"},
            ],
        )
    )
...
async def test_list_empty(mock_ctx: MagicMock) -> None:
    respx.get("http://test-mcp-api/kb").mock(return_value=Response(200, json=[]))
...
async def test_list_passes_token(mock_ctx: MagicMock) -> None:
    route = respx.get("http://test-mcp-api/kb").mock(return_value=Response(200, json=[]))
