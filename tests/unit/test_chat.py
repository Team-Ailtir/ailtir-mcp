...
async def test_chat_sends_question_in_body(mock_ctx: MagicMock) -> None:
    ...
    route = respx.post("http://test-mcp-api/kb/kb-123/chat").mock(
...
async def test_chat_missing_answer_field(mock_ctx: MagicMock) -> None:
    respx.post("http://test-mcp-api/kb/kb-123/chat").mock(return_value=Response(200, json={}))
...
async def test_chat_propagates_http_error(mock_ctx: MagicMock) -> None:
    respx.post("http://test-mcp-api/kb/kb-500/chat").mock(return_value=Response(500))
