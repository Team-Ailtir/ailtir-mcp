...
async def test_analyse_success(mock_ctx: MagicMock) -> None:
    respx.post("http://test-mcp-api/kb/kb-123/analyse").mock(return_value=Response(202))
...
async def test_analyse_passes_token(mock_ctx: MagicMock) -> None:
    route = respx.post("http://test-mcp-api/kb/kb-123/analyse").mock(return_value=Response(202))
...
async def test_analyse_propagates_http_error(mock_ctx: MagicMock) -> None:
    respx.post("http://test-mcp-api/kb/kb-404/analyse").mock(return_value=Response(404))
