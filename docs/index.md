# ailtir-mcp

An [MCP](https://modelcontextprotocol.io) plugin that gives Claude direct access to
[Ailtir](https://ailtir.ai) — upload documents, build a knowledge base, and run RAG
chat from within your AI assistant.

## Getting started

- [Installation](installation.md) — install uv and get your MCP key
- [Configuration](configuration.md) — connect the plugin to Claude Code or Claude Desktop
- [Usage](usage.md) — available tools and typical workflow
